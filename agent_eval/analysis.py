...
def _make_bucket(
    name: str, case_ids: set[str], passed_case_ids: set[str]
) -> AnalysisBucket:
    total = len(case_ids)
...
def _breakdown(
    labels_by_case: dict[str, Iterable[str]],
    passed_case_ids: set[str],
    *,
    sort_priority: bool = False,
) -> list[AnalysisBucket]:
    ...
    return [
        _make_bucket(name, grouped[name], passed_case_ids)
        for name in sorted(grouped, key=key)
    ]
...
def _suggested_investigation(
    ...
    parts = [
        f"Start with representative cases {', '.join(representative_cases) or 'none'}"
    ]
    if affected_areas:
    ...
    if assertion_type:
        parts.append(
            f"validate the {assertion_type} assertion expectations against actual responses"
        )
    else:
    ...
    if common_root_cause:
        ...
        counts = ", ".join(
            f"{cause}={count}" for cause, count in analysis["root_cause_counts"].items()
        )
...
def _case_evidence(
    case_id: str,
    failures_by_case: dict[str, FailureRecord],
...
def build_run_analysis(
    ...
    totals = AnalysisTotals(
        total=total, passed=passed, failed=failed, pass_rate=_rate(passed, total)
    )

    cases_by_id = {case.id: case for case in cases}
    case_ids = {result.case_id for result in eval_results}
    tag_labels = {
        case.id: (case.tags or ["untagged"]) for case in cases if case.id in case_ids
    }
    priority_labels = {
        case.id: [case.priority or "unprioritized"]
        for case in cases
    ...
    for cluster in clusters.clusters:
        ...
        evidence = [
            ...
            if (item := _case_evidence(case_id, failures_by_case, cases_by_id))
            is not None
        ]
        ...
        cluster_analyses.append(
            ClusterAnalysis(
                ...
                evidence=evidence,
                suggested_investigation=_suggested_investigation(
                    cluster.cluster_id,
                    common_signature,
    ...
    return RunAnalysis(
        ...
        priority_breakdown=_breakdown(
            priority_labels, passed_case_ids, sort_priority=True
        ),

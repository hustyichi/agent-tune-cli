...
from agent_eval.models import AssertionResult, AssertionSpec, EvalCase, LlmJudgeConfig, RawResult
...
class LlmJudgeProvider(Protocol):
    def evaluate(self, case: EvalCase, raw: RawResult, assertion: AssertionSpec, config: LlmJudgeConfig) -> AssertionResult:
        raise NotImplementedError


class StubLlmJudgeProvider:
    def evaluate(self, case: EvalCase, raw: RawResult, assertion: AssertionSpec, config: LlmJudgeConfig) -> AssertionResult:  # noqa: ARG002
        if config.stub_result == "pass":
            return AssertionResult(type="llm_judge", metric=assertion.metric, passed=True, score=1.0, reason="stub llm judge pass")
        if config.stub_result == "fail":
            return AssertionResult(type="llm_judge", metric=assertion.metric, passed=False, score=0.0, reason="stub llm judge fail")
        return AssertionResult(type="llm_judge", metric=assertion.metric, passed=True, skipped=True, reason="llm judge skipped by offline stub")
...
class DeepEvalJudgeProvider:
    def evaluate(self, case: EvalCase, raw: RawResult, assertion: AssertionSpec, config: LlmJudgeConfig) -> AssertionResult:
        ...
        if metric_name not in SUPPORTED_DEEPEVAL_METRICS:
            return AssertionResult(type="llm_judge", metric=metric_name, passed=False, reason=f"unsupported llm_judge metric: {metric_name}")
        input_text = _case_input(case.inputs)
        output_text = _actual_output(raw.response)
        if not input_text.strip():
            return AssertionResult(type="llm_judge", metric=metric_name, passed=False, reason="llm judge input is empty")
        if not output_text.strip():
            return AssertionResult(type="llm_judge", metric=metric_name, passed=False, reason="llm judge actual_output is empty")
        try:
            metrics_mod = importlib.import_module("deepeval.metrics")
            test_case_mod = importlib.import_module("deepeval.test_case")
            metric = metrics_mod.AnswerRelevancyMetric(threshold=config.threshold, model=config.model, include_reason=True)
            test_case = test_case_mod.LLMTestCase(input=input_text, actual_output=output_text)
        ...
        return AssertionResult(type="llm_judge", metric=metric_name, passed=passed, score=score, reason=reason)
    ...
    def evaluate(self, case: EvalCase, raw: RawResult) -> list[AssertionResult]:
        ...
        for assertion in case.assertions:
            ...
            if not self.config.enabled:
                results.append(AssertionResult(type="llm_judge", metric=assertion.metric, passed=True, skipped=True, reason="llm judge disabled"))
                continue
...
def _provider_error(metric_name: str, exc: Exception) -> AssertionResult:
    return AssertionResult(type="llm_judge", metric=metric_name, passed=False, reason=_safe_reason(f"deepeval provider error: {exc}"))

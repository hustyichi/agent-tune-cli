...
* 更细粒度 failure signature 的首个本地规则版本聚焦 RAG/工具链 root cause；更丰富执行语义断言仍作为后续 V1.5+ 候选
...
## 7.1 Mode A：Python 函数适配器模式（生成项目默认）

当前本地发布面的默认接入方式是通过 Python adapter 调用本地函数；Node adapter 仍作为后续候选，不属于当前支持面。

### 输入

adapter 函数接收完整 case 字典，关键字段包括 `id`、`inputs`、`context`、`assertions`、`expected_execution`、`evaluation_policy`。

### 输出

adapter 可直接返回 raw response，也可返回 `{response, debug_meta}`；其中 `debug_meta` 用于路由、工具调用、检索、fallback 等执行语义评估。
...
* in-process 同步执行；`runner.timeout_seconds` 不会硬杀死卡住的 Python 代码
* `runner.concurrency > 1` 时 adapter 函数需要无状态或线程安全
...
## 7.3 Mode C：脚本进程模式（高级 / 兼容路径）

通过本地命令执行目标 Agent，适合已有 CLI、非 Python Agent、需要进程隔离或 subprocess timeout 的场景。
...
* 既有脚本或 CLI 驱动型 Agent
* 非 Python / 任意命令接入
* 需要硬进程隔离或 subprocess timeout enforcement
...
当前本地规则版 failure signature 还会以兼容方式增加可选 root-cause
信息，用于表达 RAG/工具链执行语义失败，例如：
...
这些字段不改变 `agent-eval/v1alpha1` 协议值、artifact 文件名或当前
`CLUSTER_KEY_VERSION = "v1"` 聚类身份；cluster 级 root cause 以聚合分析
字段保存，避免混合根因簇被误标为单一根因。
...
* `eval.yaml`
* `cases/sample.jsonl`
* `sample_agent.py`（默认 Python function adapter 示例）
* 目录结构
...
* compare
* 本地文件落盘
* Python adapter/function（生成项目默认）、HTTP/API、脚本进程（高级/兼容）三种目标模式
* `evaluation_policy.reruns` 的本地重跑与 `attempts.jsonl` 辅助产物
* 混合评估：硬断言、执行语义评估、可选 DeepEval `answer_relevancy`
* 失败聚类
* RAG/工具链 failure signature root-cause 聚合分析（兼容 v1 cluster key）
...
* 更细粒度 failure signature（在当前 RAG/工具链 root-cause 首版基础上继续扩展）
* 更丰富的执行语义断言
* 更好的 cluster 命名

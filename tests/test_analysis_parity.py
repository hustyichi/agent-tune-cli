...
def _fixture():
        ...
        EvalResult(
            ...
            assertion_results=[
                AssertionResult(
                    type="contains",
                    passed=False,
                    reason="Expected answer to contain pricing",
                )
            ],
        ...
        EvalResult(
            ...
            assertion_results=[
                AssertionResult(
                    type="contains",
                    passed=False,
                    reason="Expected answer to contain discount",
                )
            ],
            failure_signature=signature,
        ),
    ...
    failures = [
        FailureRecord(
            run_id="r1",
            case_id="c1",
            reasons=["Expected answer to contain pricing"],
            failure_signature=signature,
            raw_status="success",
        ),
        FailureRecord(
            run_id="r1",
            case_id="c2",
            reasons=["Expected answer to contain discount"],
            failure_signature=signature,
            raw_status="success",
        ),
    ]
...
def test_build_run_analysis_computes_shared_run_and_cluster_facts():
    ...
    analysis = build_run_analysis(
        "r1", cases, raw_results, eval_results, failures, clusters
    )
    ...
    assert {bucket.name: bucket.total for bucket in analysis.tag_breakdown} == {
        "pricing": 1,
        "rag": 2,
        "smoke": 1,
    }
    assert {bucket.name: bucket.failed for bucket in analysis.tag_breakdown}["rag"] == 2
    assert {bucket.name: bucket.total for bucket in analysis.priority_breakdown} == {
        "p1": 1,
        "p2": 1,
        "p3": 1,
    }
...
def test_markdown_and_repair_json_share_analysis_values():
    ...
    assert (
        repair_cluster["analysis"]["signature_explanation"]
        == analysis.clusters[0].signature_explanation
    )
    assert (
        repair_cluster["analysis"]["suggested_investigation"]
        == analysis.clusters[0].suggested_investigation
    )
    assert (
        repair_cluster["analysis"]["affected_areas"]
        == analysis.clusters[0].affected_areas
    )
    assert (
        repair_cluster["evidence"][0]["reason"]
        == analysis.clusters[0].evidence[0].reason
    )
...
def test_analysis_evidence_redacts_secret_like_failure_reasons():
    ...
    failures[0] = failures[0].model_copy(
        update={"reasons": [f"actual token {secret} does not contain safe"]}
    )
    ...
    legacy_repair = build_repair_input("demo", "r1", clusters, failures).model_dump(
        mode="json"
    )

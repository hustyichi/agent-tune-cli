...
def run_cli(tmp_path: Path, *args: str) -> subprocess.CompletedProcess[str]:
    ...
    return subprocess.run(
        [sys.executable, "-m", "agent_eval", *args],
        cwd=tmp_path,
        text=True,
        capture_output=True,
        env=env,
...
def test_generated_project_e2e_offline(tmp_path: Path):
    ...
    cluster = run_cli(
        tmp_path, "inspect", "--run", "latest", "--cluster", clusters[0]["cluster_id"]
    )
    ...
    exported = json.loads(
        run_cli(tmp_path, "export", "--run", "latest", "--show").stdout
    )
    ...
    assert clusters[0]["common_signature"]["analysis"]["root_causes"] == [
        "route_mismatch"
    ]
    assert clusters[0]["common_signature"]["analysis"]["root_cause_counts"] == {
        "route_mismatch": 1
    }
    assert (
        clusters[0]["common_signature"]["analysis"]["common_root_cause"]
        == "route_mismatch"
    )
    ...
    same_run_compare = json.loads(
        run_cli(
            tmp_path, "compare", "--base", "latest", "--target", "latest", "--show"
        ).stdout
    )
...
""".strip()
    ...
    (tmp_path / "cases" / "adapter.jsonl").write_text(
        '{"id":"a1","inputs":{"query":"hello"},"assertions":[{"type":"contains","target":"$.answer","expected":"hello"}],"expected_execution":{"expected_route":"adapter","must_call_tools":["local.run"]}}\n'
    )
...
def test_evaluation_policy_reruns_write_attempt_sidecar_and_preserve_consumers(
    tmp_path: Path,
):
    (tmp_path / "cases").mkdir()
    (tmp_path / "runs").mkdir()
...
def test_redaction_in_e2e_artifacts(tmp_path: Path):
    ...
    with (tmp_path / "cases" / "sample.jsonl").open("a") as fh:
        fh.write(
            '{"id":"secret_case","inputs":{"query":"route knowledge pricing","api_key":"SENTINEL_SECRET"},"assertions":[{"type":"contains","target":"$.answer","expected":"pricing"}]}\n'
        )
    run_cli(tmp_path, "run")
    run_id = (tmp_path / "runs" / "latest.txt").read_text().strip()
    combined = "\n".join(
        p.read_text() for p in (tmp_path / "runs" / run_id).iterdir() if p.is_file()
    )
...
class JsonHandler(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        self.wfile.write(
            json.dumps(
                {
                    "response": {"answer": f"echo {payload['query']}"},
                    "debug_meta": {"route": "http", "tool_calls": []},
                }
            ).encode()
        )
...
artifacts:
  ...
  reports_dir: ./reports
""")
        (tmp_path / "cases" / "http.jsonl").write_text(
            '{"id":"h1","inputs":{"query":"hello"},"assertions":[{"type":"contains","target":"$.answer","expected":"hello"}],"expected_execution":{"expected_route":"http"}}\n'
        )
        run_cli(tmp_path, "run")
        run_id = (tmp_path / "runs" / "latest.txt").read_text().strip()
        ...
        assert (
            manifest["config_snapshot"]["target"]["http"]["headers"]["Authorization"]
            == "[REDACTED]"
        )
    finally:
        server.shutdown()
...
""")
    (tmp_path / "cases" / "http.jsonl").write_text(
        '{"id":"h1","inputs":{"query":"hello"},"assertions":[{"type":"contains","target":"$.answer","expected":"hello"}]}\n'
    )
...
def test_export_inspect_reject_run_traversal(tmp_path: Path):
    ...
    for command in ("export", "inspect"):
        proc = subprocess.run(
            [sys.executable, "-m", "agent_eval", command, "--run", "../outside"],
            cwd=tmp_path,
            text=True,
            capture_output=True,
            env=env,
...
def test_compare_generated_runs_e2e(tmp_path: Path):
    ...
    sample.write_text(
        sample.read_text().replace("no retrieval please", "route knowledge pricing")
    )
    run_cli(tmp_path, "run", "--run-name", "target")

    shown = run_cli(
        tmp_path, "compare", "--base", "baseline", "--target", "target", "--show"
    )
    ...
    transitions = {
        item["case_id"]: item["transition"] for item in comparison["case_transitions"]
    }
...
def test_generated_project_runs_without_python_on_path(tmp_path: Path):
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "agent_eval", "run"],
        cwd=tmp_path,
        text=True,
        capture_output=True,
        env=env,
